...
if [[ $EUID -eq 0 ]]; then
   echo "This script should not be run as unprivileged user" 1>&2
   exit 1
fi
...
sudo pacman -Rsn --noconfirm kdebase-konqueror kdebase-konq-plugins kate 
...
#TODO startkde before do that or create arboresence first ?
...
# Permissions problems (root not writable by user) ?
